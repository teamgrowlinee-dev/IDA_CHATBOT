...
app.use("/simulator-assets", express.static(path.join(publicDir, "simulator"), { maxAge: 0 }));

const threeDir = path.resolve(__dirname, "../../node_modules/three");
app.use("/vendor/three", express.static(threeDir, { maxAge: "7d" }));

app.get("/room", (req, res) => {
  const projectId = String(req.query.projectId ?? "").trim();
  const target = projectId
    ? `/planner?projectId=${encodeURIComponent(projectId)}&panel=room-edit`
    : "/planner?panel=room-edit";
  res.redirect(302, target);
});

app.get("/planner", (_req, res) => {
  res.sendFile(path.join(publicDir, "simulator", "planner-workspace.html"));
});

app.get("/planner-legacy", (_req, res) => {
  res.sendFile(path.join(publicDir, "simulator", "planner.html"));
});

app.get("/simulator", (req, res) => {
  const projectId = String(req.query.projectId ?? "").trim();
  const target = projectId
    ? `/planner?projectId=${encodeURIComponent(projectId)}&panel=catalog`
    : "/planner?panel=catalog";
  res.redirect(302, target);
});
